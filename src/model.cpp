...
Model::Model(std::string const &path, bool flipTexture) {
    log = Logger("Model");
    ...
    if (!scene || scene->mFlags 
        ...
        log.error(std::string("ASSIMP:: ", importer.GetErrorString()));
    ...
    log.info("loading scene: " + std::string(scene->mName.data));
...
void Model::processNode(aiNode *node, const aiScene *scene) {
    log.info(
        "loading node: " + std::string(node->mName.data)
        + ", meshes " + std::to_string(node->mNumMeshes)
        + ", childs " + std::to_string(node->mNumChildren));
...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene) {
    log.info(
        "loading mesh: " + std::string(mesh->mName.data)
...
std::vector<Texture> Model::loadMaterialTextures(
    ...
    for (GLuint i = 0; i < mat->GetTextureCount(type); i++) {
        ...
        texture.flipped = flipTexture;
        
        textures.push_back(texture);
        
        texturesInfo.insert(texture);
    }
    return textures;
}
